//! Pure, dependency-free input validation for the auth endpoints.
//!
...
//! failure, so the handler layer maps them to a 422 with that code and the
//! whole module can later move verbatim to `apps/shared`. Written test-first
//! per CLAUDE.md's TDD process.
