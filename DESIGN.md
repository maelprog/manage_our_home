...
**Ce document ne porte plus de valeur mesurée** (#95). Les garde-fous
d'`apps/web/src/app.rs` calculent déjà le poids de la feuille, celui des
déclarations, les deux plafonds et la marge qui reste ; ils en sont désormais
la seule source, et une commande les imprime :
...
plus** : tout nombre qui décrit l'état courant du dépôt. Le motif est mesuré :
la vérification de #72 a compté cinq tours, et à chaque tour le défaut était
dans la prose chiffrée de ce document, jamais dans le code — `style.css` est
