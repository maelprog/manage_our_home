...
est ressortie à 8 133 o — l'estimation était juste (voir ci-dessous).*
...
| `/agenda` | 13 288 o | **3 272 o** | −10 016 |
| `/stocks` | 12 726 o | **2 696 o** | −10 030 |
| `/recipes` | 12 628 o | **2 679 o** | −9 949 |
| `/grocery-list` | 12 887 o | **2 898 o** | −9 989 |
| `/budget` | 12 475 o | **2 489 o** | −9 986 |
| `/messagerie` | **18 093 o** | **8 133 o** | −9 960 |
| `/groups` | 10 750 o | **902 o** | −9 848 |

**`/messagerie` rentre dans le budget** pour la première fois depuis qu'il
existe : 8 133 o contre 14 336, soit 6 203 o de marge, sans qu'une ligne de
...
La feuille, elle, est désormais une réponse à part : 26 893 o bruts, **9 949 o
gzip** et 10 455 o zstd mesurés localement, 10 255 o / 10 495 o réellement
reçus à travers Caddy (qui compresse bien le `text/css` et laisse passer le
`Cache-Control: immutable` — vérifié). Payés **une fois par visiteur et par
déploiement**, l'URL portant l'empreinte du contenu.

#### L'aller-retour qu'on réintroduit, chiffré

C'est le bénéfice n°1 ci-dessus, et il est réel : sur un cache froid, le
navigateur parse le document, découvre le `<link>`, demande la feuille et
attend avant de peindre. **Coût : un aller-retour, une seule fois.**

En octets, la bascule est déjà rentable à la **première** page vue : sur
`/agenda`, avant, 13 288 o ; après, 3 272 + 9 949 = 13 221 o. Les deux se
valent. À partir de la deuxième page de la session, le visiteur économise
~9 950 o à chaque navigation. Le coût n'est donc pas un volume, c'est une
**sérialisation** : deux allers-retours avant le premier pixel au lieu d'un,
et seulement au tout premier chargement.
...
Face à ~9 950 o épargnés par page vue ensuite — ~16 ms à 5 Mb/s — l'aller-retour
est remboursé par la deuxième navigation.
...
|---|---|---|---|
| Réponse complète compressée, routes principales | **≤ 14 KiB** (14 336 o) | 8 133 o au pire (`/messagerie`), 684 – 3 272 sur les sept autres | alléger le **document** — la feuille n'y est plus |
| Feuille compressée | **≤ 14 KiB** (14 336 o), *était 10 KiB* | 9 983 o | découper la feuille ; il n'y a plus de `/assets` où la déplacer |
...
**Seuil 2 — la feuille : sa dérivation tombe, le garde-fou reste et son
chiffre monte à 14 KiB.** C'est la seule vraie concession de #89, et elle est
écrite ici plutôt que subie. 10 KiB n'était pas un jugement sur les feuilles
de style, c'était une soustraction : 14 336 − le document le plus lourd
(3 248 o sur `/agenda`) = 11 088, arrondi à la baisse. Cette soustraction n'a
plus d'objet quand le document voyage sans la feuille. Et l'ancienne consigne
— « ce plafond ne se relève pas, le dépassement veut dire qu'on prend la porte
de sortie » — est *épuisée*, pas contredite : la porte a été prise, il n'y a
pas de troisième endroit où mettre la feuille. Restait à choisir entre
supprimer le garde-fou et le redériver. Il est redérivé : la feuille est
maintenant sa **propre** réponse bloquante, et ce qui mérite d'être garanti
est qu'elle arrive en un seul aller-retour — le même IW10, donc 14 336 o. Ce
que ça concède : la feuille peut grossir d'un tiers avant que quoi que ce soit
ne se déclenche. Ce que ça évite : perdre la pression vers la sobriété que ce
document crédite d'avoir rendu #66 et #68 nécessaires. Le test
...
**Seuil 3 — les déclarations : inchangé, et c'est lui qui récupère du
mordant.** Même valeur, même motif, même test. Sa calibration est un *ordre* —
il doit se déclencher avant le plafond de feuille pour que la pression tombe
sur les règles et jamais sur la prose — et cet ordre tient tant que le plafond
de feuille reste au-dessus de `3 072 / (2 995 / 9 983)` = 10 239,66 o. Après
#71 il restait **0,34 octet** de marge : un octet de commentaire de plus et le
dispositif s'inversait. Avec un plafond de feuille à 14 336, la marge redevient
**4 096 octets**. #89 ne répare donc pas cette marge en dégraissant quoi que ce
soit ; il la répare en donnant à la feuille une fenêtre à elle. Il reste **77
octets de déclarations** à se partager pour #72–#74, exactement comme avant :
#89 n'ajoute pas une règle.
...
l'inlining vraie, et c'est encore lui qui fait tenir la feuille de 26 893 o
bruts dans 10 255 o sur le fil.
...
| 2026-08-03 | Plafond de feuille : **10 KiB → 14 KiB**, dérivation refaite (#89) | La seule concession du lot, déclarée. Les 10 KiB étaient une soustraction (14 336 − le document le plus lourd) qui n'a plus d'objet quand la feuille ne voyage plus dans le document, et la consigne « ce plafond ne se relève pas, on prend la porte de sortie » est épuisée : la porte a été prise. Le garde-fou est redérivé plutôt que supprimé — la feuille est maintenant sa propre réponse bloquante et doit tenir dans un aller-retour, donc le même IW10. Coût : la feuille peut grossir d'un tiers avant que rien ne se déclenche. Test renommé `the_compressed_stylesheet_still_arrives_in_one_round_trip` |
| 2026-08-03 | Plafond des déclarations : **inchangé à 3 KiB** (#89) | Même valeur, même motif, même test — et c'est lui qui récupère le mordant. Sa calibration est un ordre (mordre avant le plafond de feuille) dont la marge d'inversion était tombée à **0,34 octet** après #71 ; un plafond de feuille à 14 336 la ramène à **4 096 octets**. Réparé en donnant une fenêtre à la feuille, pas en dégraissant une règle ni un commentaire. #89 n'ajoute aucune déclaration : les 77 octets restants pour #72–#74 sont intacts |
