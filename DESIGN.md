...
**Une exception, et elle est étroite : le lot en cours peut corriger en place
un motif *faux* qu'il vient lui-même d'écrire.** Tant qu'une entrée appartient
à une PR en vol elle n'est pas encore de l'histoire — le dépôt fusionne en
squash, elle atterrira dans le même commit que le reste — et empiler un renvoi
sur une affirmation que personne n'a jamais lue hors de cette branche coûte au
lecteur sans rien préserver. Trois conditions : la décision, sa date et ses
valeurs ne bougent pas, **seul le motif est réécrit** ; l'entrée **porte la
marque de sa réécriture**, pour que l'historique se lise dans le document et
pas seulement dans `git log` ; et une entrée datée dit ce qui a été corrigé.
Une entrée **atterrie** ne bénéficie pas de l'exception, même fausse : celle
du 2026-07-30 sur `/messagerie` l'est sur un point et le reste, avec son
renvoi. Ce lot s'en est servi ; les entrées concernées portent la marque au
bout de leur motif, et c'est à cette marque qu'on les reconnaît — pas à un
décompte dans ce paragraphe, qui périmerait au commit suivant. Une marque
**ne cite pas de sha** : les commits de branche disparaissent au squash, et
un renvoi vers un commit injoignable est une marque morte. Elle nomme la date
de la réécriture et ce qu'elle a changé. Et comme le renvoi, la marque est une
**addition** : la poser ne réécrit rien, et n'appelle donc pas de marque à son
tour.
...
| 2026-08-29 | La convention de ce journal énonce son exception : un récit **faux** se corrige en place (#72) | La convention posée en tête — « une entrée datée n'est jamais réécrite, ce qu'on peut lui ajouter est un renvoi » — était démentie par le lot qui l'a écrite. Inventaire refait ligne à ligne sur les huit révisions de la branche qui touchent ce fichier : **quatre réécritures en place, par trois commits** — l'entrée du 2026-08-05 sur le plafond des déclarations, réécrite **deux fois** ; celle du 2026-08-05 sur le `<script>` inline ; celle du 2026-08-06 sur le plafond de feuille. Les ajouts de **renvoi** ne comptent pas : la convention les autorise, et il y en a quatre autres. L'acte est validé et n'est pas défait, mais **sur un seul argument** : aucune de ces entrées n'avait quitté la branche, et la PR atterrit en un commit — empiler un renvoi sur un motif que personne n'avait lu ailleurs aurait coûté au lecteur sans rien préserver. La brièveté du délai, elle, ne fonde rien et a été retirée : il va de **4 h 30** à **21 j 22 h** (l'entrée du 2026-08-06, réécrite le 2026-08-28). Ce qui manquait, c'est que la règle n'énonçait aucune exception et que **trois réécritures sur quatre ne portaient aucune marque**. Au squash les commits de branche disparaissent : un lecteur de `main` aurait lu ces lignes comme des entrées datées jamais retouchées, sans plus aucun moyen de le savoir — un futur auteur aurait lu la règle comme absolue, un futur vérificateur l'aurait comparée à l'historique et aurait conclu à une infraction commise par le lot qui l'écrit. L'exception est donc écrite et **bornée** — elle ne vaut que pour une entrée du lot en cours, la décision, sa date et ses valeurs sont immuables, seul le motif est réécrit, la marque est obligatoire et **ne cite aucun sha** (un sha de branche est injoignable après le squash), et une entrée datée dit ce qui a été corrigé. Une entrée atterrie n'en bénéficie pas. **Les trois marques manquantes sont posées** |
