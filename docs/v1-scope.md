...
| 10 | Object storage (MinIO) wiring | **done** | Wired as part of epic #2 (Agenda): `src/storage.rs` (aws-sdk-s3 client pointed at MinIO), `infra/docker-compose.yml` minio service. Fridge-scan photos are post-v1. Object lifecycle closed in three passes: deleting an event (#54/#56) and deleting a group (#57/#59) now remove the objects, not just the rows, and the `reconcile-attachments` binary (#58, `src/attachment_reconcile.rs`) sweeps up whatever leaked before those landed — plus the low-rate drip from `upload_attachment`, which writes the object before the row commits and so can't be made airtight. Dry-run by default; requires the `BYPASSRLS` `ADMIN_DATABASE_URL` role, because `event_attachments` is FORCE-RLS'd and reads back **empty** on a normal connection, which would classify the whole bucket as orphaned. See `apps/api/README.md`. |
| 11 | Transactional email (Mailjet via `lettre`) | partially designed | Stack decided (architecture.md; provider settled on Mailjet, #136); actual sending code is part of Auth epic (verification, password reset) — check epic #1 branch for implementation status. |
