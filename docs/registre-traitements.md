...
Une ligne vit donc au plus une heure de plus que sa durée. La passe tourne
sur `ADMIN_DATABASE_URL` : `invitations` est sous une politique RLS forcée,
et sans `BYPASSRLS` la passe refuse de tourner (erreur journalisée à chaque
heure) plutôt que de ne rien supprimer en se disant réussie. Elle ne tourne
que tant que l'API tourne.
