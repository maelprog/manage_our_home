...
| 2 | Agenda | événements, tâches, pièces jointes (photos/documents), rappels | Planification familiale | Exécution du contrat | Tant que l'événement/le compte existe ; supprimé avec le groupe ou anonymisé (`created_by`) à la purge du compte auteur ; un fichier de pièce jointe orphelin (sans ligne en base) est supprimé par un balayage quotidien dès qu'il a plus de 24 h | Aucun tiers |
